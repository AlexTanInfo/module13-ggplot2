...
# What do you see?
diamond.plot <- ggplot(data = diamonds.sample)
diamond.plot

# Draw a scatter plot (with point geometry) with for the `diamonds.sample` set,
# with the `carat` mapped to the x-position and `price` mapped to the y-position.
diamond.plot <- ggplot(data = diamonds.sample, mapping = aes(x = carat, y = price)) +
  geom_point()
...
ggplot(data = diamonds, mapping = aes(x = carat, y = price, size = price)) +
  geom_point()
...
ggplot(data = diamonds.sample) +
  geom_bar(mapping = aes(x = cut, color = carat))
